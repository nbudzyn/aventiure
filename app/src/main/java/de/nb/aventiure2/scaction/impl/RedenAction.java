...
public class RedenAction<TALKER extends IDescribableGO & ITalkerGO>
    ...
    Collection<RedenAction<TALKER>> buildActions(
        ...
        return buildActions(db, gos, initialStoryState,
                talker,
                talkSteps);
    }
    ...
    Collection<RedenAction<TALKER>> buildActions(final AvDatabase db,
        ...
        for (final SCTalkAction talkStep : talkSteps) {
            if (stepTypeFits(gos, talker, talkStep.getStepType())) {
                res.add(buildAction(db, gos, initialStoryState,
                        talker,
    ...
    private static boolean stepTypeFits(final GameObjectService gos,
        ...
        if (isDefinitivDiskontinuitaet(gos, talker)) {
    ...
    RedenAction<TALKER> buildAction(final AvDatabase db,
        ...
        final PraedikatOhneLeerstellen praedikatOhneLeerstellen =
                fuelleGgfPraedikatLeerstelleMitCreature(gos, talkStep.getName(), talker);

        return buildAction(db, gos, initialStoryState, talker,
                talkStep,
                praedikatOhneLeerstellen);
    }

    private static PraedikatOhneLeerstellen fuelleGgfPraedikatLeerstelleMitCreature(
            final GameObjectService goService,
        ...
        if (praedikat instanceof PraedikatMitEinerObjektleerstelle) {
            final Nominalphrase creatureDesc =
                    goService.getPOVDescription(SPIELER_CHARAKTER, talker, true);
    ...
    RedenAction<TALKER> buildAction(final AvDatabase db,
                                    ...
                                    final SCTalkAction talkStep,
                                    final PraedikatOhneLeerstellen praedikatOhneLeerstellen) {
        return new RedenAction<>(db, gos, initialStoryState, talker,
                talkStep,
    ...
    protected boolean isDefinitivDiskontinuitaet() {
        return isDefinitivDiskontinuitaet(gos, talker);
    }

    private static boolean isDefinitivDiskontinuitaet(final GameObjectService goService,
                                                      final ITalkerGO talker) {
        ...
        return goService.loadSC().memoryComp().lastActionWas(Action.Type.REDEN, talker) &&
